...
resource "abbey_grant_kit" "abbey_demo_site" {
  ...
  workflow = {
    steps = [
      {
        reviewers = {
          all_of = ["arvil+test1@abbey.io", "arvil.nagpal@gmail.com"] # CHANGEME
        }
      }
    ]
  }
  ...
  policies = [
    { bundle = "github://arviln/quickstart-v4/policies" } # CHANGEME
  ]

  output = {
    # Replace with your own path pointing to where you want your access changes to manifest.
    # Path is an RFC 3986 URI, such as `github://{organization}/{repo}/path/to/file.tf`.
    location = "github://arviln/quickstart-v4/access.tf" # CHANGEME
    append = <<-EOT
